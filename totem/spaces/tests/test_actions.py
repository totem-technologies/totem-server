import uuid

from django.contrib.messages import get_messages

from totem.users.tests.factories import UserFactory

from .. import actions
from ..actions import SubscribeSpaceAction
from .factories import SpaceFactory


class TestActions:
    def test_join_session_action(self, db):
        event_slug = "slug"
        user = UserFactory()
        assert user.verified is False
        action = actions.JoinSessionAction(user, parameters={"session_slug": event_slug})
        assert action.action_id == "spaces:join"
        url = action.build_url()
        assert f"spaces/join/{event_slug}" in url
        assert "?token=" in url
        # get it back
        token = url.split("?token=")[1]
        action = actions.JoinSessionAction(user, parameters={"session_slug": event_slug})
        user, parameters = action.resolve(token)
        assert user.verified is True
        assert user == user
        assert parameters["session_slug"] == event_slug

    def test_join_expired(self, db):
        event_slug = "slug"
        user = UserFactory()
        action = actions.JoinSessionAction(user, parameters={"session_slug": event_slug})
        url = action.build_url(expires_at="2021-01-01T00:00:00Z")
        token = url.split("?token=")[1]
        try:
            actions.JoinSessionAction.resolve(token)
            assert False
        except actions.JoinSessionAction.ActionExpired:
            assert True
        except Exception:
            assert False

    def test_join_does_not_exists(self, db):
        try:
            actions.JoinSessionAction.resolve(str(uuid.uuid4()))
            assert False
        except actions.JoinSessionAction.ActionDoesNotExist:
            assert True


class TestSubscribeAction:
    def test_subscribe(self, client, db):
        user = UserFactory()
        space = SpaceFactory()
        assert space.subscribed.count() == 0
        url = SubscribeSpaceAction(user=user, parameters={"space_slug": space.slug, "subscribe": True}).build_url()
        assert "spaces/subscribe/" in url
        assert space.slug in url
        token = url.split("token=")[1].split("&")[0]
        user1, params = SubscribeSpaceAction.resolve(token)
        assert user1 == user
        assert params
        assert params["space_slug"] == space.slug
        assert params["subscribe"] is True
        response = client.get(url)
        space.refresh_from_db()
        assert space.subscribed.count() == 1
        message = list(get_messages(response.wsgi_request))[0]
        assert "subscribed" in message.message.lower()

    def test_unsubscribe(self, client, db):
        user = UserFactory()
        space = SpaceFactory()
        space.subscribed.add(user)
        assert space.subscribed.count() == 1
        url = SubscribeSpaceAction(user=user, parameters={"space_slug": space.slug, "subscribe": False}).build_url()
        assert "spaces/subscribe/" in url
        assert space.slug in url
        token = url.split("token=")[1].split("&")[0]
        user1, params = SubscribeSpaceAction.resolve(token)
        assert user1 == user
        assert params
        assert params["space_slug"] == space.slug
        assert params["subscribe"] is False

        response = client.get(url)
        space.refresh_from_db()
        assert space.subscribed.count() == 0
        message = list(get_messages(response.wsgi_request))[0]
        assert "unsubscribed" in message.message.lower()

    def test_invalid_token(self, client, db):
        user = UserFactory()
        space = SpaceFactory()
        url = SubscribeSpaceAction(user=user, parameters={"space_slug": space.slug, "subscribe": False}).build_url()
        assert "spaces/subscribe/" in url
        assert space.slug in url
        token = url.split("token=")[1].split("&")[0]
        token = token[:-1] + "0"
        response = client.get(url[:-3])  # make the token invalid
        assert response.status_code == 302
        assert response.url == f"/spaces/{space.slug}/"
        message = list(get_messages(response.wsgi_request))[0]
        assert "invalid" in message.message.lower()
